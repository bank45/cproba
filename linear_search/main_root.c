...
// 1. ГЛОВАЛЬНЫЕ ПЕРЕМЕННЫЕ (Требование 7)
int iterations_f12 = 0;
int iterations_f23 = 0;
int iterations_f13 = 0;

// 2. ОБЪЯВЛЕНИЕ МАТЕМАТИЧЕСКИХ ФУНКЦИЙ (Должны быть строго выше любого их использования!)
float f1(float x) { 
    return 0.6f * x + 3.0f; 
}
...
void plot_gnuplot(float x13, float x23, float x12) {
    ...
    (void)status; // Чистим варнинг неиспользованного результата system
}
